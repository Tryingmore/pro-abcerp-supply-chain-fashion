...
class ValidationForwardWizard(models.TransientModel):
    ...
    def add_forward(self):
        ...
        prev_comment.write(
            {"comment": _("转到 %s") % self.forward_reviewer_id.display_name+',原因:'+self.forward_description}
        )
        ...
        review = self.env["tier.review"].create(
            {
                "model": rec._name,
                "res_id": rec.id,
                "sequence": round(max(prev_reviews.mapped("sequence")) + 0.1,1),
                "requested_by": self.env.uid,
                "request_date": prev_reviews.request_date,
                "definition_id": prev_reviews.definition_id.id,
