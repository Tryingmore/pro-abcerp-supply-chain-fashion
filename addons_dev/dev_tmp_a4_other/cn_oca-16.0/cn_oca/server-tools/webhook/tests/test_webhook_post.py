...
class Webhook(models.Model):
    ...
    def run_wehook_test_get_foo(self):
        ...
        """
        self.ensure_one()
        data = self.env.request.httprequest.data
        if json.loads(data)['foo'] != 'bar':
...
class TestWebhookPost(HttpCase):
    ...
    def setUp(self):
        ...
        #TODO register event code for tests.
        self.webhook.create({
            'name':'0_webhook_test'
        })

    def get_webhook_url(self, url='/webhook', webhook_name="wehook_test"):
        """
    ...
    def post_webhook_event(self, event, url, data, remote_ip=None, headers=None, params=None):
        """
    ...
    def test_webhook_get_foo(self):
        ...
        """
        json_response = self.post_webhook_event('get_foo', self.url, {'foo': 'bar'})
        self.assertEqual(json_response.get('error', False), False, 'Error in webhook get foo test!.')
    ...
    def test_webhook_search_with_request(self):
        ...
        self.assertEqual(wh.id, self.env.ref('webhook.webhook_test').id, "Search webhook from request IP info is not working")
